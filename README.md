...
- **Enable/Disable Surcharge**: When enabled, a payment surcharge is calculated and shown to the customer during credit card checkout before the order is placed.
...
### 10. Payment Surcharge

Merchants can add a configurable surcharge to credit card payments. The surcharge is calculated dynamically through Bluesnap and shown to the customer for confirmation before the order is placed.

**How It Works**:
Once enabled, entering credit card details on the checkout confirmation page shows a **"Calculate surcharge and proceed to payment"** button instead of submitting immediately. Calculating the surcharge shows the customer the additional amount, which is added to the order as a separate line item. This works both for a newly entered card and for a customer's saved/vaulted card, including when switching cards mid-checkout.

**Steps**:
1. Enable **Surcharge** in the plugin configuration.
2. During checkout, the customer enters their credit card details (or uses a saved card).
3. The customer clicks **"Calculate surcharge and proceed to payment"** to see the surcharge amount.
4. The customer confirms and completes the order with the surcharge included.
...
- Yes, enable the Surcharge option in the plugin configuration. The surcharge is calculated at checkout and shown to the customer before the order is placed.
