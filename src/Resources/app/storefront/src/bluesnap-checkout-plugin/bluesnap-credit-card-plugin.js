...
export default class BluesnapCreditCardPlugin extends window.PluginBaseClass {
    ...
    _registerElements() {
        ...
        this.originalPfToken = this.pfToken;
        ...
        this.securedBaseAmount = this.parentCreditCardWrapper.getAttribute('data-secured-base-amount');
        this.securedCurrency = this.parentCreditCardWrapper.getAttribute('data-secured-currency');
        this.surchargeLabel = this.parentCreditCardWrapper.getAttribute('data-surcharge-label') || 'Payment Surcharge';
        this.isSurchargeActive = this.parentCreditCardWrapper.getAttribute('data-is-surcharge-active');
        this.locale = document.documentElement.lang || 'en-GB';
        this.shopwareSubmitWrapper = document.getElementById('shopware-submit-wrapper');
    ...
    _registerEvents() {
        ...
        const surchargeBtn = document.getElementById('bluesnap-calculate-surcharge');
        if (this.isSurchargeActive && surchargeBtn) {
            surchargeBtn.addEventListener('click', (e) => {
                e.preventDefault();
                this._calculateSurcharge();
            });
        }
        const changeCardCheckbox = document.getElementById('bluesnap-change-card');
        if (changeCardCheckbox) {
            changeCardCheckbox.addEventListener('change', () => this._onChangeCardToggle());
        }
    }

    async _onChangeCardToggle() {
        const isChecked = document.getElementById('bluesnap-change-card')?.checked;
        const controlDisplay = document.getElementById('controll-display');
        const successElem = document.getElementById('bluesnap-surcharge-success');
        if (isChecked) {
            if (controlDisplay) controlDisplay.classList.remove('d-none');
            if (successElem) successElem.classList.add('d-none');
            if (this.shopwareSubmitWrapper) this.shopwareSubmitWrapper.classList.add('d-none');
            const pfInput = document.getElementById('bluesnap-pf-token-surcharge');
            const cardTypeInput = document.getElementById('bluesnap-card-type-surcharge');
            const tokenInput = document.getElementById('bluesnap-surcharge-token');
            const amountInput = document.getElementById('bluesnap-surcharge-amount');
            if (pfInput) pfInput.value = '';
            if (cardTypeInput) cardTypeInput.value = '';
            if (tokenInput) tokenInput.value = '';
            if (amountInput) amountInput.value = '';
            await this._refreshHostedFieldsWithFreshToken();
        }
    }
    ...
    _calculateSurcharge() {
        ...
        bluesnap.hostedPaymentFieldsSubmitData(
            async (callback) => {
                ...
                const cardType = callback.cardData?.ccType || 'CREDIT';
                const amount = parseFloat(this.securedAmount);
                const body = { pfToken, amount, cardType };
                const result = await BlueSnapApi.calculateSurcharge(body);
                const controlDisplay = document.getElementById('controll-display');
                const successElem = document.getElementById('bluesnap-surcharge-success');
                const errorElem = document.getElementById('error-message');
                const isSaveCardChecked = document.getElementById('bluesnap-save-card')?.checked ?? false;
                this.saveCard = isSaveCardChecked;

                if (!result?.success || !result?.message?.surchargeInfo) {
                    if (errorElem) {
                        errorElem.querySelector('p').innerText = 'Failed to calculate surcharge';
                ...
                const surchargeAmount = parseFloat(result.message.surchargeInfo.surchargeAmount || 0);
                const surchargeToken = result.message.surchargeInfo.surchargeToken || '';
                const surchargeTokenInput = document.getElementById('bluesnap-surcharge-token');
                const surchargeAmountInput = document.getElementById('bluesnap-surcharge-amount');
                const pfTokenInput = document.getElementById('bluesnap-pf-token-surcharge');
                const cardTypeInput = document.getElementById('bluesnap-card-type-surcharge');
                if (surchargeTokenInput) surchargeTokenInput.value = surchargeToken;
                if (surchargeAmountInput) surchargeAmountInput.value = surchargeAmount.toFixed(2);
                if (pfTokenInput) pfTokenInput.value = pfToken;
                if (cardTypeInput) cardTypeInput.value = cardType;

                const saveCardInput = document.getElementById('bluesnap-save-card-surcharge');
                if (saveCardInput) {
                    saveCardInput.value = isSaveCardChecked.toString();
                }

                this.threeDSecureObj.amount = parseFloat(this.securedAmount) + surchargeAmount;

                if (controlDisplay) controlDisplay.classList.add('d-none');
                if (successElem) {
                    document.getElementById('bluesnap-surcharge-success-text').innerText =
                        `Surcharge ${surchargeAmount} calculated successfully.`;
                    successElem.classList.remove('d-none');
                }
                if (errorElem) errorElem.classList.add('d-none');
                if (this.shopwareSubmitWrapper) this.shopwareSubmitWrapper.classList.remove('d-none');
                document.getElementById('bluesnap-change-card').checked = false;
    ...
    async _showForm() {
        const isSaveCardChecked = this.saveCardCheckbox ? this.saveCardCheckbox.checked : false;
        ...
        if (isSaveCardChecked) {
            ...
            this._restoreOriginalPfTokenIfNeeded();
        } else if (this.vaultedId) {
            await this._refreshHostedFieldsWithFreshToken();
        }
    }

    _restoreOriginalPfTokenIfNeeded() {
        if (!this._freshTokenFetched || !this.originalPfToken) {
            return;
        }
        this._freshTokenFetched = false;
        this.pfToken = this.originalPfToken;
        this.blueSnapObject.token = this.originalPfToken;
        this.parentCreditCardWrapper.setAttribute('data-pf-token', this.originalPfToken);
        bluesnap.hostedPaymentFieldsCreate(this.blueSnapObject);
    }

    async _refreshHostedFieldsWithFreshToken() {
        if (this._freshTokenFetched) {
            return;
        }
        const result = await BlueSnapApi.getPfToken();
        if (!result || !result.success || !result.message) {
            console.error('BlueSnap: failed to fetch fresh pfToken for card change', result);
            return;
        }
        this._freshTokenFetched = true;
        this.pfToken = result.message;
        this.blueSnapObject.token = this.pfToken;
        this.parentCreditCardWrapper.setAttribute('data-pf-token', this.pfToken);
        bluesnap.hostedPaymentFieldsCreate(this.blueSnapObject);
    }
    ...
    async _surchargeCapture() {
        const pfToken = document.getElementById('bluesnap-pf-token-surcharge')?.value;
        const cardType = document.getElementById('bluesnap-card-type-surcharge')?.value || 'CREDIT';
        const surchargeToken = document.getElementById('bluesnap-surcharge-token')?.value;
        const surchargeAmount = document.getElementById('bluesnap-surcharge-amount')?.value ?? '0';
        const body = {
            pfToken,
            amount: String(this.securedAmount),
            firstName: document.getElementById('bluesnap-first-name')?.value || this.securedFirstName,
            lastName: document.getElementById('bluesnap-last-name')?.value || this.securedLastName,
            saveCard: (document.getElementById('bluesnap-save-card-surcharge')?.value === 'true') || 
                   (document.getElementById('bluesnap-save-card-confirm')?.checked) || 
                   (document.getElementById('bluesnap-save-card')?.checked) || false,
            cardType,
            surchargeToken,
            surchargeAmount,
        };
        const loaderEl = document.getElementById('bluesnap-loader');
        if (loaderEl) {
            loaderEl.style.display = 'block';
        }
        this._scrollToBluesnapSection();

        const flow = this.parentCreditCardWrapper.getAttribute('data-flow');
        ...
        if (result && result.success) {
            ...
            const parsedMessage = typeof message === 'string' ? (() => { try { return JSON.parse(message); } catch { return [{ description: message }]; } })() : [{ description: String(message) }];
            const description = parsedMessage[0]?.description || 'Capture failed';
    ...
    _creditCardCapture() {
        const pfTokenSurcharge = document.getElementById('bluesnap-pf-token-surcharge')?.value;
        const surchargeToken = document.getElementById('bluesnap-surcharge-token')?.value;
        const hasSurcharge = pfTokenSurcharge && surchargeToken;
        if (hasSurcharge) {
            this._surchargeCapture();
            return;
        }
    ...
    async _onOrderSubmitButtonClick(event) {
        ...
        this._scrollToBluesnapSection();
        const useSavedCard = document.getElementById("bluesnap-is-save-card")?.checked === true;
        document.getElementById("bluesnap-is-save-card").classList.add('d-none');
        if (useSavedCard) {
    ...
    async _vaultedCapture() {
        ...
        const surchargeAmount = parseFloat(document.getElementById('bluesnap-surcharge-amount')?.value || '0');
        const previouslyUsedCard = {
            "last4Digits": Shopper4Digits,
            "ccType": cardType,
            "amount": parseFloat(this.securedAmount) + surchargeAmount,
            "currency": this.securedCurrency,
        };
    ...
    _getSurchargePayload() {
        const surchargeToken = document.getElementById('bluesnap-surcharge-token')?.value;
        const surchargeAmount = document.getElementById('bluesnap-surcharge-amount')?.value;
        const payload = {};

        if (surchargeToken) {
            payload.surchargeToken = surchargeToken;
        }

        if (surchargeAmount !== undefined && surchargeAmount !== null && surchargeAmount !== '') {
            payload.surchargeAmount = surchargeAmount;
        }
